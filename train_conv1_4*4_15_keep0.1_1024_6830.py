...
'''
W_conv2 = weight_variable([4, 4, 30, 15])
...
W_conv3 = weight_variable([2, 2, 16, 64])
#variable_summaries(W_conv2)
...
for i in range(100000):
  if i%100 == 0:
    ...
    print('Test accuracy with 1.0 at step %s: %s' % (i, acc))
    acc = sess.run(accuracy, feed_dict={x: images_test.images, y_: labels_test.labels, keep_prob: 0.5})
    print('Test accuracy with 0.5 at step %s: %s' % (i, acc))
    acc = sess.run(accuracy, feed_dict={x: images_test.images, y_: labels_test.labels, keep_prob: 0.1})
    print('Test accuracy with 0.1 at step %s: %s' % (i, acc))
  ...
  summary, _ = sess.run([merged, train_step], feed_dict={x: images_train.images, y_: labels_train.labels, keep_prob: 0.1})
